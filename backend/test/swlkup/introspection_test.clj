...
(deftest introspection
  (is (= (->> (-> (graphql {:query "{__schema{types{name}}}"})
              ...
              (map :name)
              sort)
         '("Boolean" "Contacts" "ContactsInput" "Float" "ID" "Int" "Location" "Long" "MutationType" "QueryType" "String" "SupervisorInput" "languages" "lookup" "ngo" "offers" "supervisor_get" "supervisors"))))
